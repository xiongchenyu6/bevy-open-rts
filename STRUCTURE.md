...
- `scripts/comfy_hunyuan3d_queue.py` stages reference images and API workflows for remote ComfyUI/Hunyuan3D runs. `CryoSprayer`, `FlameAssaultBuggy`, and `LongbowMissileCrawler` are completed single-GLB replacements mapped under `assets/models/hunyuan3d/`; the quality gate now leaves 11 remaining multipart unit replacements in the queue.
